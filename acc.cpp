...
int imax = 100000;
int nexp = 10000; //numero pseudoesperimenti
//double stheta;
//double sphi;
...
void acc(){
 ...
 TH1F* hacc = new TH1F("dis_acc","Distribuzione accettanza", 100, 0.22, 0.32);
 TFile rfile("accettanza.root","RECREATE");
  ...
  for (int a = 1; a <= nexp; a++) { 
    cout << "Experiment n. " << a << ": " << endl;
      ...
      reps1 = eps1 + rndgen.Uniform(2*s_eps1)-s_eps1; //inserire distribuzione corretta
      reps2 = eps2 + rndgen.Uniform(2*s_eps2)-s_eps2;
      reps3 = eps3 + rndgen.Uniform(2*s_eps3)-s_eps3;
    for (int i = 1; i <= imax;) {
